...
def train(X_train, y_train):
    ...
    tree_model = DecisionTreeClassifier(max_depth=5, random_state=1912)
    # log_reg.fit(X_train, y_train)
    tree_model.fit(X_train, y_train)

    ### Log the model with the input and output schema
    # Infer signature (input and output schema)
    signature = mlflow.models.infer_signature(X_train, tree_model.predict(X_train))

    # Log model
    mlflow.sklearn.log_model(tree_model, "model", signature=signature)
...
def main():
    ...
    with mlflow.start_run():
        ...
        ### Log tag
        mlflow.set_tag("model_type", "tree_classifier")
        ...
        mlflow.log_artifact("confusion_matrix.png")  # Log the file as an artifact
